//Phần khai báo class
var ramArr = [2, 3, 4, 6, 8, 12];
var romArr = [8, 16, 32, 64, 128, 256];

class Product {
    static totalProduct = 0; // danh dau
    constructor(name, price_old, price, colorList, imgList, brand) {
        ...
        this.ram = ramArr[Math.floor(Math.random() * ramArr.length)];
        this.rom = romArr[Math.floor(Math.random() * romArr.length)];
...
class data {
    ...
    static getProductAtPage(list, page, max_products_show = 10) {
...
class order {
    ...
    static addBuyNowOrder(productId, soluong, productColor, userid, orderTime) {
        ...
        try {
            ...
            orderList.push(new Order(myCart, userid, Date.now()));
            order.loadOrder(orderList);
            history.addNode(Date.now(), "Người dùng " + user.getUserId(userid).username + " đã đặt 1 đơn hàng!");
            return true;
        }
...
class menu {
    static filterProudct(list, brandList, priceMin, priceMax, ramList, romList, search) {
        // const list = data.getProducts();
        if (!list)
            return null;
        var result = list;
        ...
        //       data.filterProductRam(result, ramList[i])
        //     );
        //   }
        //   subResult = tempList;
    ...
    static filterProudct(list, brandList, priceMin, priceMax, ramList, romList, search) {
        ...
        if (brandList.length > 0) {
            ...
            for (let i = 0; i < brandList.length; i++) {
                ...
                tempList = tempList.concat(data.filterProductBrand(result, brandList[i]));
            }
            result = tempList;
        }
        // console.log(result);
...
class time {
    ...
    static getDateTimeStr(timeSpan) {
        ...
        var str = date.getDate() + '/' + (date.getMonth() + 1) + '/' + date.getFullYear() + "  " + time.getHourSecStr(timeSpan);
        return str;
    }
